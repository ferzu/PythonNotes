...
j = 0
count = 0
for year, directors in nominated.items(): # making an ordered list of unique directors
    dr += directors
dr_set = set(dr)
dr_list = sorted(list(dr_set))
while i < len(dr_list): # counting nominated films per director. Storing result as a tuple in the list director_counts - works fine
    for year, directors in nominated.items():
        while j < len(directors):
            if dr_list[i] == directors[j]:
                count += 1
            j += 1
        j = 0
    director_counts.append((dr_list[i], count))
    count = 0
    i += 1
# print (director_counts)
i =0
j = 0
values = [] # creating a dictionary type {year: (director, #nominations)} - it does not work
while i < len(dr_list):
    for year, directors in nominated.items():
        while j < len(directors):
            if director_counts[i][0] == directors[j]:
                nom_count_dict[year] = values.append(director_counts[i])
            j += 1
        j = 0
    i += 1
print (nom_count_dict)
director_counts = [(Alberto, 5), (Joan, 1), etc ]
...
# Solution: **************************************************************************************************************
# 1. a) Create a dictionary that includes the count of Oscar nominations for each director in the nominations list.
nominated = {year: list_directors[}
nom_count_dic = {director: count_nominations}
nom_count_dic ={}
for year, list_directors in nominated.items():
    for director in list_directors:
        if director not in nom_count_dic:
            nom_count_dic[director] = 1
        else:
            nom_count_dic[director] += 1
print(nom_count_dic)
# The solution asked something else: Question 1a) Provide a dictionary with the count of nominations for each director.
...
for year, list_dir in nominated.items():
    for director in list_dir:
        if director not in nom_count_dict:
            ...
            nom_count_dict[director] += 1
#Question 1b: Provide a dictionary with the count of wins for each director.
