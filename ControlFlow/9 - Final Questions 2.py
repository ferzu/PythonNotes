# 2. Provide a list with the name(s) of the director(s) with the most Oscar wins. We are asking for a list because there could be more than 1 director tied for the most Oscar wins.
...
most_oscar_wins = [(name, count_wins), (name2,count_wins, etc]
# list_dir = list(winners.values())
# director_oscar = [ (director, list_dir.count(director)) for years, director in winners]
list_winners = []
for years, director in winners.items():
    list_winners.append(director)
list_winners = [ director for years, director in winners.items()]
print(list_winners)
list_winners = ["".join(director) for years, director in winners.items()]
for director in range(len(list_winners)):
    count = list_winners.count(list_winners[director])
    list_winners[director] = (list_winners[director], count)
dict_oscars = dict(set(list_winners))
maxkey = max(dict_oscars, key = lambda k: dict_oscars[k])
list1 = []
for director, value in dict_oscars.items():
    if dict_oscars[director] == maxkey:
        list1.append(director)
print(list1)
...
# Solution: ************************************************************************************************************************************
# 1. Here's the logic for the solution:
